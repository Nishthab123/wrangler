   ...
   public List<Directive> parse() throws RecipeException {
     ...
     try {
       ...
       new GrammarWalker(new RecipeCompiler(), context).walk(recipe, (command, tokenGroup) -> {
         ...
         try {
           // Check if the token group contains ByteSize or TimeDuration
           for (Object token : tokenGroup) {
             if (token instanceof ByteSize) {
               // Handle ByteSize token
               ByteSize byteSize = (ByteSize) token;
               // You can add logic to store the ByteSize or pass it to relevant directive
             } else if (token instanceof TimeDuration) {
               // Handle TimeDuration token
               TimeDuration timeDuration = (TimeDuration) token;
               // You can add logic to store the TimeDuration or pass it to relevant directive
             }
           }
